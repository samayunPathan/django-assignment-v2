...
#         """Display locations and their types as a comma-separated list."""
#         return ", ".join([f"{location.name} ({location.get_type_display()})" for location in obj.locations.all()])
...
# Image Inline for PropertyAdmin
class ImageInline(admin.TabularInline):
    model = Image
    extra = 2
    fields = ('image', 'image_preview', 'created_at', 'updated_at')
    ...
    def image_preview(self, instance):
        """Display image preview in the inline admin."""
        if instance.image:
            return mark_safe(f'<img src="{instance.image.url}" width="100" height="100" />')
        return "No image"
    image_preview.short_description = 'Preview'
...
class PropertyAdmin(admin.ModelAdmin):
    ...
    def image_preview(self, obj):
        """Display image previews for the property."""
        images = obj.images.all()
        if images.exists():
            images_html = ''.join([
                f'<div style="flex: 0 0 auto; margin-right: 10px;">'
                f'<img src="{image.image.url}" style="width: 80px; height: 80px; max-width: 100px; max-height: 100px;" />'
                f'</div>'
                for image in images[:2]
            ])
            if images.count() > 2:
...
class ImageAdmin(admin.ModelAdmin):
    ...
    def image_preview(self, obj):
        """Display image preview in the admin."""
        if obj.image:
            image_url = obj.image.url if hasattr(obj.image, 'url') else obj.image
            return mark_safe(f'<img src="{image_url}" width="100" height="100" />')
        return 'No image'
    ...
    def image_path(self, obj):
        ...
        return obj.image if obj.image else 'No image path'
    image_path.short_description = 'Image Path'
